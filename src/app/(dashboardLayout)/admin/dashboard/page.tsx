...
const AdminDashboard = () => {
  ...
  const [tourListing, setTourListing] = useState<any>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    (async () => {
      setLoading(true);
      const bookingRes = await getBookingStatsService();
      const paymentRes = await getPaymentStatsService();
      const userRes = await getUserStatsService();
      const tourListingRes = await getListingStatsService();
      ...
      if (tourListingRes.success && tourListingRes.data) {
        setTourListing(tourListingRes.data);
      }
      setLoading(false);
    })();
  }, []);
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    ...
                    dataKey="count"
                  >
                    {payment?.totalPaymentByStatus?.map((entry: any) => (
                      <Cell
                        key={`cell-${entry._id}`}
                        fill={
                          statusColors[
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    ...
                    dataKey="count"
                  >
                    {user?.usersByRole?.map((entry: any) => (
                      <Cell
                        key={`cell-${entry._id}`}
          ...
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent>
              <div className="space-y-4">
                {tourListing?.totalHighestBookedTour?.map((tour: any) => (
                  <div
                    key={tour._id}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent className="space-y-3">
              {booking?.totalBookingByStatus?.map((item: any) => (
                <div
                  key={item._id}
