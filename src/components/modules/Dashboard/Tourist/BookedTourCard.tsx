  ...
  return (
    <div className="p-3 bg-white rounded-xl shadow-2xl border border-gray-100 transition duration-300 hover:shadow-3xl">
      ...
      <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-sm text-gray-700">
        ...
        {/* Row 1: Date & Location */}
        <div>
          <p className="text-xs font-medium text-gray-500 uppercase">Tour Date</p>
          <p className="text-base font-semibold text-indigo-600">{formattedDate}</p>
        </div>
        <div>
          <p className="text-xs font-medium text-gray-500 uppercase">Location & Category</p>
          <p className="text-base font-medium">{tourListingId?.city} • <span className="text-orange-500">{tourListingId?.category}</span></p>   
        </div>
        
        {/* Row 2: Guide & Tourist */}
        <div>
          <p className="text-xs font-medium text-gray-500 uppercase">Your Guide</p>
          <p className="font-medium text-gray-900">{guideId?.name}</p>
          <p className="text-xs text-gray-500 truncate">{guideId?.email}</p>
        </div>
        <div>
          <p className="text-xs font-medium text-gray-500 uppercase">Tourist Name</p>
        ...
        <div>
          <p className="text-xs font-medium text-gray-500 uppercase">Group Size</p>
          <p className="text-base font-medium">{groupSize} Person{groupSize > 1 ? 's' : ''}</p>
        </div>
        <div>
      ...
      <div className="mt-2 pt-2 border-t border-gray-200 flex justify-between items-center">
        ...
        <div className="text-right space-y-1">
          ...
          <span className={`inline-flex items-center px-3 py-1 bg-green-50 text-green-700 text-xs font-medium rounded-md ${paymentId.status === 'PAID' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            <svg className="w-3 h-3 mr-1.5" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path>
            </svg>
            {paymentId?.status}
          </span>
          <p className="text-xs text-gray-400 mt-1">Transaction: {paymentId?.transactionId}</p>
        </div>
      </div>
    </div>
  );
